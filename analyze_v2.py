...
def classify(para, pysbd_sents, punkt_sents):
    ...
    _pysbd_headers = [i for i, s in enumerate(pysbd_sents) if is_header(s)]
    _punkt_headers = [i for i, s in enumerate(punkt_sents) if is_header(s)]
    ...
    for name, sents in [("pysbd", pysbd_content), ("punkt", punkt_content)]:
        ...
        for i, s in enumerate(sents):
            ...
            if stripped in (".", "..", "...", "....") and i > 0:
                issues[name].append(
                    f"Orphan punctuation fragment '{stripped}' at [{i}]"
                )
        ...
        if pysbd_norm[i] != punkt_norm[i]:
            ...
            if p_sent.startswith(k_sent[:20]) and len(p_sent) > len(k_sent):
                ...
                if k_sent.rstrip()[-1] == '"' or k_sent.rstrip()[-1] == "\u201d":
                    issues["punkt"].append(
                        f"Possible false split at end of quote at [{i}]"
                    )
    ...
    if pe == 0 and ke == 0:
        ...
        if np == nk:
            ...
            return (
                "UNCLEAR",
                f"{more} splits more ({np} vs {nk}), no clear errors",
                issues,
            )
    elif pe < ke:
...
for i, rec in enumerate(data["disagreements"]):
    verdict, explanation, issues = classify(
        rec["paragraph"], rec["pysbd"], rec["punkt"]
    )
