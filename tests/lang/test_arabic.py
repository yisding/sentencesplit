...
GOLDEN_AR_RULES_TEST_CASES = [
    ...
    # Arabic comma "،" is a within-sentence separator, NOT a terminator — a
    # deliberate divergence from pragmatic_segmenter / pySBD, which split at every
    # "،". This comma-chained run-on has no terminal punctuation, so it stays a
    # single segment instead of being fragmented (the upstream rule even cut the
    # "...ضغط الدم، والقلب" noun phrase in half).
    (
    ...
    # A plain comma-separated list stays whole; only the period ends the sentence.
    (
        "أحمد، علي، ومحمد أصدقاء. هم يدرسون معا.",
        ["أحمد، علي، ومحمد أصدقاء.", "هم يدرسون معا."],
    ),
]
