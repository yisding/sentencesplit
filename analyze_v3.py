...
for i, rec in enumerate(data["disagreements"]):
    ...
    print(
        f"#{i + 1:>2} [{rec['article'][:20]:<20}] {len(ps):>2}v{len(ks):<2} {pattern}"
    )
    ...
    if div is not None and div < min_len:
        ...
        if div + 1 < len(ps):
            print(
                f"    pySBD[{div + 1}]: {ps[div + 1][:80]}{'...' if len(ps[div + 1]) > 80 else ''}"
            )
        if div + 1 < len(ks):
            print(
                f"    Punkt[{div + 1}]: {ks[div + 1][:80]}{'...' if len(ks[div + 1]) > 80 else ''}"
            )
    elif div >= min_len:
