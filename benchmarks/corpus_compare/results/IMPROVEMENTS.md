...
- **chained-initials-before-capital:** A 3+ part single-letter initialism (`F.J.G.`) re-added a boundary before a capitalized following word. Added `_is_initials_name()` to treat `Initials + Surname` as a personal name and suppress the boundary, while still allowing the article-preceded acronym case (`the S.A.T.`) and clear capital-follower boundary case (`H.B.S. She`).
