...
class Segmenter:
    ...
    def cleaner(self, text: str):
        if hasattr(self.language_module, "Cleaner"):
            return self.language_module.Cleaner(
                text, self.language_module, doc_type=self.doc_type
            )
        else:
    ...
    def processor(self, text: str):
        if hasattr(self.language_module, "Processor"):
            return self.language_module.Processor(
                text, self.language_module, char_span=self.char_span
            )
        else:
    ...
    def segment(self, text: str | None) -> List[str] | List[TextSpan]:
        ...
        if self.char_span:
            return [
                TextSpan(s, start, end)
                for s, start, end in self._match_spans(
    ...
    def segment_spans(self, text: str | None) -> List[TextSpan]:
        ...
        seg = Segmenter(
            language=self.language, clean=False, doc_type=self.doc_type, char_span=True
        )
    ...
    def segment_clean(self, text: str | None) -> List[str]:
        ...
        seg = Segmenter(
            language=self.language, clean=True, doc_type=self.doc_type, char_span=False
        )
        return seg.segment(text)
