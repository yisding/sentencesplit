...
class ListItemReplacer:
    ...
    def replace_periods_in_numbered_list(self):
        self.scan_lists(
            self.NUMBERED_LIST_REGEX_1, self.NUMBERED_LIST_REGEX_2, "♨", strip=True
        )
    ...
    def format_alphabetical_lists(self):
        self.text = self.add_line_breaks_for_alphabetical_list_with_periods(
            roman_numeral=False
        )
        self.text = self.add_line_breaks_for_alphabetical_list_with_parens(
            roman_numeral=False
        )
        return self.text
    ...
    def add_line_breaks_for_alphabetical_list_with_periods(self, roman_numeral=False):
        txt = self.iterate_alphabet_array(
            self.ALPHABETICAL_LIST_WITH_PERIODS, roman_numeral=roman_numeral
        )
        return txt

    def add_line_breaks_for_alphabetical_list_with_parens(self, roman_numeral=False):
        txt = self.iterate_alphabet_array(
            self.ALPHABETICAL_LIST_WITH_PARENS, parens=True, roman_numeral=roman_numeral
        )
        return txt
        ...
        for ind, (item, pos) in enumerate(list_array):
            ...
            if not found_forward and ind > 0:
                ...
                if pos - prev_pos < 200 and (
                    ((item - 1) == prev_item)
                    or ((item == 0) and (prev_item == 9))
                    or ((item == 9) and (prev_item == 0))
                ):
                    self.substitute_found_list_items(regex2, item, strip, replacement)
    ...
    def substitute_found_list_items(self, regex, each, strip, replacement):
        ...
        self.text = re.sub(
            regex,
            partial(replace_item, val=each, strip=strip, repl=replacement),
            self.text,
        )
    ...
    def last_array_item_replacement(
        self, a, i, alphabet, alphabet_index, list_array, parens
    ):
        if (
            (len(alphabet) == 0)
    ...
    def iterate_alphabet_array(self, regex, parens=False, roman_numeral=False):
        ...
        for ind, each in enumerate(list_array):
            if ind == len(list_array) - 1:
                self.text = self.last_array_item_replacement(
                    each, ind, alphabet, alphabet_index, list_array, parens
                )
            else:
