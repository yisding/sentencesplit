...
tests = [
    ...
    ("version 2 merge", "The Linux kernel is licensed under the GPL, version 2. The GPL requires that anyone who distributes software must share source code."),
...
print("\n\nwhite man merge:")
result = seg.segment("I had never heard this slur used by a more sophisticated Negro, or by any white man. I tried to convey this effect.")
for i, s in enumerate(result):
    print(f"  [{i}] {s}")
...
print("\n\ncomponents.) merge:")
result = seg.segment("The Internet protocol suite (also called TCP/IP, based on the first two components.) This is a suite of protocols.")
for i, s in enumerate(result):
    print(f"  [{i}] {s}")
