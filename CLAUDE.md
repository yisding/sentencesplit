...
- `processor.py` — Core logic, organized into two explicit pipelines returned by `_text_processing_phases()` (newline normalization → list-item markers → abbreviation replacement → optional CJK abbreviation rules → numbers / continuous punctuation / numeric refs → special-token protection) and `_boundary_processing_phases()` (terminal marker → exclamation words → between-punctuation → double/quotation punctuation → list parens). After splitting, `split_into_segments()` post-processes, restores placeholders, re-splits at Latin `.) Capital` or CJK quote boundaries, and merges orphan fragments. Reads `split_mode` ("conservative" / "balanced" / "aggressive") as a global split-bias for ambiguous boundaries (default "balanced" reproduces the historically tuned output). Tunable decision points compare `utils.split_mode_rank(self.split_mode)` instead of hard-coding a single lean; structural rules (decimals, period-before-comma, known abbreviations) ignore it.
- `language_profile.py` — `LanguageProfile` frozen dataclass, resolved per-language via `LanguageProfile.from_language(lang)`. Centralizes the most-used language-specific hooks the Processor needs: `abbreviation_replacer_cls`, `between_punctuation_cls`, `list_item_replacer_cls`, `cjk_abbreviation_rules`, `cjk_reporting_clause_re`, `colon_rule`, `comma_rule`, `latin_uppercase_resplit` flag, and compiled regexes (`sentence_boundary_re`, `quotation_end_re`, etc.). The Processor reads these through `self.profile`. The static rule hooks (`Punctuations`, `Numbers`, `DoublePunctuationRules`, `EllipsisRules`, `SubSymbolsRules`, the special-token rules) are also resolved onto the profile in `LanguageProfile._build` and read via `self.profile.*` — the Processor no longer reads `self.lang.*` at all. Of these, only `Punctuations` and `Numbers` are actually overridden by shipping languages.
